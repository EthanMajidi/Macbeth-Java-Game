...
/**
 * Created by EthanMajidi on 2018-04-02.
 */
...
public class King extends Enemy {
    ...
    public King(PlayScreen screen, float x, float y) {
        super(screen, x, y);
        frames = new com.badlogic.gdx.utils.Array<TextureRegion>();
        for(int i = 0; i < 2; i++)
            frames.add(new TextureRegion(screen.getAtlas().findRegion("King"), i * 16, 0, 16, 16));
    ...
    public void update(float dt){
        ...
        if(setToDestroy && !destoryed){
            ...
            destoryed = true;
            setRegion(new TextureRegion(screen.getAtlas().findRegion("King"), 32, 0, 16, 16));
            stateTime = 0;
        }
        else if (!destoryed) {
