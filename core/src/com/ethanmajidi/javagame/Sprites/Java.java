...
public class Java extends Sprite {
    ...
    public Java(PlayScreen screen){
        ...
        for(int i = 1; i < 4; i++)
            frames.add(new TextureRegion(screen.getAtlas().findRegion("Macbeth"), i * 16, 0, 16, 16));
        ...
        playerJump = new TextureRegion(screen.getAtlas().findRegion("Macbeth"), 64,0,16,16);
        ...
        playerStand = new TextureRegion(screen.getAtlas().findRegion("Macbeth"), 0, 0, 16, 16);
        bigPlayerStand = new TextureRegion(screen.getAtlas().findRegion("big_mario"), 0,0,16,32);

        //create dead player texture region
        playerDead = new TextureRegion(screen.getAtlas().findRegion("Macbeth"), 80, 0, 16, 16);
        playerWon = new TextureRegion(screen.getAtlas().findRegion("Macbeth"), 96, 0, 16, 16);
        //define player
        defineMario();
    ...
    public State getState(){
        //test to see where the player is
        //if player is going positive on the y axis he is jumping
        if(playerIsDead)
            return State.DEAD;
        if (playerHasWon)
            return State.WON;
